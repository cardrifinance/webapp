...
import { useBankModal } from "@/stores/overlay";
...
export function BankAutocomplete({
  ...
  const { open, setOpen, setBandDetails } = useBankModal();
              ...
              {filteredBanks?.map((bank) => (
                <div
                  ...
                  className={cn(
                    ...
                    field.value === bank.bankCode && "bg-blue-100"
                  )}
                  onClick={() => {
                    setBandDetails(bank);
                    field.onChange(bank.bankCode);
                    setOpen(false);
                    setSearchTerm("");
                  }}
                >
