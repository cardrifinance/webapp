...
const TransactionPin = () => {
  ...
  const onSubmit = async () => {
    ...
    const toastId = toast.loading("Updating transaction pin");
    try {
      setIsSubmitting(true);

      await setTransactionPIN(otp);

      toast.success("Transaction Pin succesfully.", { id: toastId });
      ...
      toast.error(
        ...
        error?.response.data.message ||
          "Something went wrong. Failed to updating pin",
        {
          id: toastId,
        }
      );
    ...
    <div className="w-full max-w-[522px] flex flex-col gap-[42px]  justify-center items-center   pt-32 ">
      <div className="w-full text-center">
        ...
        <span className="text-base font-normal font-inter text-center  text-[#464646] leading-[28px] mt-4 inline-block">
          Create a 4-digit PIN to authorize your transaction securely
        </span>
      </div>
